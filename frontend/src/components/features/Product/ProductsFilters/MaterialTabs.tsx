...
interface MaterialTabsProps {
    /** collectionId sélectionné (chaîne) ou null pour "Toutes". */
    value: string | null;
...
export default function MaterialTabs({ value, onChange, className = "" }: MaterialTabsProps) {
    ...
    const tabs: { id: string | null; label: string }[] = [
        ...
        ...collections.map((c) => ({ id: String(c.id), label: toLabel(c.material, c.title) })),
    ];

    const isActive = (id: string | null) => (value ?? null) === id || (id === null && value === "all");
